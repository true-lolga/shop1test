...
class afo_forgot_pass_class {
	...
	public function ForgotPassForm(){
		...
		?>
			<div id="log_forms" class="forgot_pass">
	...
	public function message_close_button(){
		$cb = '<a href="javascript:void(0);" onclick="closeMessageFp();" class="close_button_afo">x</a>';
		return $cb;
	}
	
	public function load_script(){?>
		<script type="text/javascript">
			function closeMessageFp(){jQuery('.error_wid_login').hide();jQuery('.success_wid_login').hide();}
		</script>
	<?php }
...
function forgot_pass_validate(){
	...
	if(isset($_GET['key']) && $_GET['action'] == "reset_pwd") {
		...
		if(!empty($reset_key) && !empty($user_data)) {
			...
			if ( !wp_mail($user_email, __('Password Reset Request', 'login-sidebar-widget'), $message, $headers) ) {
	...
	if(isset($_POST['option']) and $_POST['option'] == "afo_forgot_pass"){
		...
		$user_username = sanitize_text_field(trim($_POST['user_username']));
